...
impl <T: PartialOrd + Clone> PointerWaveletTree<T> {
    ...
    pub fn access(&self, i: usize) -> Option<&T> {
     ...
     pub fn select(&self, q: T, i: u64) -> Option<&T> {
        // If the given index is larger than the size of the bitmap (i.e. if it is larger than the amout of symbols in
        ...
        if i as u64 >= self.root.bitmap.bits().len() {
            ...
            // Find out the position of q in the alphabet
           // let q_position : Option<usize> = self.alphabet.iter().position(|x| x == &q);
            // If q could not be found in the alphabet, return None. Otherwise, return select_q(i).
            //i.and_then(|position| self.root.select(position as u64, q, i, 0, self.alphabet.len()))
            
            // Find out the i-th position of q in the alphabet
            let position_in_alphabeth : Option<usize> = self.alphabet.iter().position(|x| x == &q);
            
            
            //= self.root.select(q: T, i as u64, 0, self.alphabet.len());
              
               // If some position is returned, return the corresponding index at the root.
               position_in_alphabeth.and_then(|position| self.root.select(position as u64, q, i, 0, self.alphabet.len()))
               //position_in_alphabeth.map(|q| &self.alphabet.iter().position(|x| x == &q))    
...
impl WaveletTreeNode {
    ...
    fn select(&self, q: T, i: u64, a: usize, b: usize) -> Option<u64> {
     ...
     let middle = (a+b)/2
      if self.is_empty{
          None
              ...
              if q > Some(middle) {
                   self.right_child.as_ref().and_then(|child| child.select(btmp.select_1(i).unwrap() - 1, middle, b))
              }
             
              else if { 
                  self.left_child.as_ref().and_then(|child| child.select(btmp.select_0(i).unwrap() - 1, a, middle)) 
              }
             else{ Some(middle)  }
          }
